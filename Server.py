...
try:
    ...
    # Continue running so the API can at least return errors properly
...
def index():
    return jsonify({
        ...
        "endpoints": {
            ...
            "/api/detect": "Detect emotions in uploaded images",
            "/api/video": "Process video frames for emotion detection",
            "/api/models": "Get information about loaded models"
        }
    })
...
def process_video_frame():
    ...
    result_pil = Image.fromarray(result_image)
    buffer = io.BytesIO()
    result_pil.save(buffer, format='JPEG', quality=75)  # Lower quality for video frames
    result_base64 = base64.b64encode(buffer.getvalue()).decode('utf-8')
...
def health_check():
    ...
    models_loaded = hasattr(app, 'detector') and hasattr(app, 'predictor') and hasattr(app, 'model')
...
if __name__ == '__main__':
    ...
    if not os.path.exists('face_landmarks.dat'):
        print("Warning: face_landmarks.dat not found. Please download it.")
    
    if not os.path.exists('emotion.h5'):
        print("Warning: emotion.h5 not found. Please download it.")
